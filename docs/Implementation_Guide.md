...
2. **Classifier Agent (`agents/classifier.py`):**
   ...
   - **Usage via API:**
     ```bash
     curl "http://localhost:8000/agent/classifier?INPUT_TEXT=Hello,%20how%20are%20you?"
...
3. **Summarizer Agent (`agents/summarizer.py`):**
   ...
   - **Usage via API:**
     ```bash
     curl "http://localhost:8000/agent/summarizer?TEXT_TO_SUMMARIZE=FastAPI%20is%20an%20efficient%20framework.%20It%20simplifies%20API%20development."
